...
Early Router는 SPIQA train/calibration에서 threshold `0.3281827436`으로 동결했다. Calibration Strong route는 38.30%였지만 SciVQA test에서는 89.65%로 증가해 실제 query skip이 10.35%에 그쳤다. 품질 retention은 R@1 105.48%, MRR 104.26%로 통과했지만 15% 비용 Gate는 실패했다.

SciVQA test one-shot Gate 결과는 다음과 같다.
...
전체 판정은 quality PASS / cost FAIL이다. 같은 test에서 threshold나 특징을 변경하지 않는다. 다음 버전은 새 development corpus에서 candidate count의 절대 규모에 의존하지 않는 특징과 cost-sensitive objective를 사용하고, 새로운 unopened holdout에서만 인증한다.
