...
> 확대된 locked selector는 독립 81문항에서 Caption R@1을 67.9%에서 71.6%로 높이고 ColSmol 호출을 12.35%로 제한했다. 그러나 95% 신뢰구간이 0을 포함하므로 통계적 성공으로 확정하지 않는다. 다음 단계는 SPIQA test-A 재튜닝이 아니라 외부 멀티모달 데이터셋의 독립 인증이다.
